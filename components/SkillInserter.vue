...
const removeSkill = async (key: number, id: number) => {
  ...
  const { error } = await supabase.from(props.tableName).delete().eq("id", id);
};
...
const saveFileToDb = async (id: number) => {
  ...
  if (!data) return;
  await supabase.from(props.tableName).upsert({ id, file: data.path }).select();
};
      ...
      >
        ...
        <div>
          <div v-for="field in fields" class="field">
            <label class="font-bold my-2 block">{{ field.label }}</label>
            <textarea
            ...
            <div v-if="field.fieldType === 'relation'">
              <div
                class="flex gap-2"
                v-for="item in relations[field.tableKey]"
                :key="item.id"
              >
                <input
                  v-model="skill[field.tableKey]"
                  ...
                  :value="item.id"
                />
                <label :for="`item-${skill.id}-${item.id}`">{{
